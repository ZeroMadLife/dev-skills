...
This skill is designed to work across multiple AI coding assistants (Claude Code, Codex, Cursor, Gemini). Tool requirements are generalized:

**Required capabilities:**
- **File read/write**: Use your agent's file reading and writing tools
  - Claude: `Read` / `Write` / `Edit`
  - Codex: file read / apply_patch / write_file
  - Cursor: file view / file edit
- **Shell execution**: Use your agent's shell/bash tool to run bundled Python scripts
  - Claude: `Bash`
  - Codex: `exec_command`
  - Cursor: terminal commands

**Optional capability:**
- **SubAgent/parallel agents**: When available, use for semantic `sortin` or `resort` maintenance that spans many files. Use the main agent directly only when subagents are unavailable.

**Note on scripts**: The bundled Python scripts (in `scripts/` directory) are tool-agnostic. They operate on files in `.ustht/` and can be invoked via any shell execution tool.
